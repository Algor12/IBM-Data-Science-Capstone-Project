...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        suc = filtered_df[filtered_df['class'] == 1].shape[0]
        fail = filtered_df.shape[0] - suc
        fig = px.pie(values = [suc, fail],
                    names = ['Successes', 'Failures'],
                    title = f'Successes and Failures for {entered_site} launch site')
...
@app.callback(Output(component_id = 'success-payload-scatter-chart', component_property = 'figure'),
                [Input(component_id = 'payload-slider', component_property = 'value'), 
                 Input(component_id = 'site-dropdown', component_property = 'value')])
def get_scatter_plot(entered_range, entered_site):
